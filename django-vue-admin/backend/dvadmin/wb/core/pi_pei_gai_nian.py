...
def setColor3Purple(chuang_data, qx, lian_ban_code):
    ...
    chuang_data = sorted(chuang_data, key=lambda x: (x[1]["lianbantianshuyesterday"], x[1]["zhangdie4thday"]), reverse=True)
...
def definedcolor1(chuang_data, bu_zhang_data, qx, is_chuang_ye=1):
    for (key, item) in chuang_data:
        ...
        if item["jianguanleixingyesterday"] != "" or \
                ...
                (is_chuang_ye != 1 and item["ziyouliutongshizhiyesterday"] / 10000 / 10000 > 100 and gp.getCode(item["code"]) !=
