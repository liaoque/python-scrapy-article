...
class Command(BaseCommand):
    ...
    def saveD(self, d):
        ...
        if SharesZhangTings.objects.filter(code_id=d["股票代码"], date_as=formatted_date, morning=1).count():
            return
    ...
    def saveD2(self, d):
        date_obj = datetime.strptime(d["date"], '%Y%m%d')
        formatted_date = date_obj.strftime('%Y-%m-%d')
        if SharesBlockGns.objects.filter(code_id=d["指数代码"], date_as=formatted_date,morning=1).count():
            return
