...
class Command(BaseCommand):
    ...
    def etf(self):
        ...
        if self.A50():
            self.etf.append("A50")
        if self.hs300():
            self.etf.append("hs300")
        if self.zt500():
            self.etf.append("zt500")
        if self.zt1000():
            self.etf.append("zt1000")
        if self.zt2000():
            self.etf.append("zt2000")
    ...
    def gn(self):
        ...
        :return:
        """
        gns = rediangainian.rdgainian(data)
        if len(gns) == 0:
            return
        self.gns = list(set(gns))
    ...
    def xg(self):
        ...
        """
        for gn in self.gns:
            codes = rediangainnian.codes(gn, self.etf)
            # 计算macd
            for code in codes:
