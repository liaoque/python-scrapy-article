...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for i in range(4):
            codes = zhangTing.zhangTingGns(t2,i+1)
            for item in codes:
                d = self.initD2(item, t2)
                self.saveD2(d)


    def initD(self, item, t):
        d = {}
        d["股票代码"] = item["股票代码"][:-3]
        d["股票简称"] = item["股票简称"]
        d["所属概念"] = item["所属概念"]
        ...
        for key, value in item.items():
            if "首次涨停时间[%s]"%(t) in key:
                d["首次涨停时间"] = value.replace(" ", "")
                d["date"] = key[-9:-1]
            if "几天几板[%s]"%(t) in key:
                d["几天几板"] = value
            if "连续涨停天数[%s]"%(t) in key:
                d["连续涨停天数"] = value

            if "最终涨停时间[%s]"%(t) in key:
                d["最终涨停时间"] = value.replace(" ", "")
            if "a股市值(不含限售股)[%s]"%(t) in key:
                d["a股市值流通市值"] = value

        return d


    def initD2(self, item, t):
        d = {}
        d["指数代码"] = item["指数代码"][:-3]
        d["指数简称"] = item["指数简称"]
        for key, value in item.items():
            if "指数@开盘价:不复权[%s]"%(t)  in key:
                d["开盘价"] = value
                d["date"] = key[-9:-1]
            if "指数@收盘价:不复权[%s]"%(t) in key:
                d["收盘价"] = value
            if "指数@最低价:不复权[%s]"%(t)  in key:
                d["最低价"] = value
            if "指数@最高价:不复权[%s]"%(t)  in key:
                d["最高价"] = value
            if "指数@涨跌幅:前复权[%s]"%(t)  in key:
                d["涨跌幅"] = value
        return d
