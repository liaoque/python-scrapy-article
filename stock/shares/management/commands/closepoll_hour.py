...
class Command(BaseCommand):
    ...
    def calculateKdj(self):
        ...
        for item in SharesName.objects.filter(status=1,code_type=1):
            ...
            # 写过了
            code = item.code
            sharesKdjList = SharesHours.objects.filter(code_id=code, date_as=today)
    ...
    # 
    #     result = np.array(SharesHourKdj.objects.filter(date_as__lte=today).values('date_as').annotate(counts=Count(id)))[-5:]
    #     if len(result) < 4:
    #         return
    #     if len(result) == 4:
