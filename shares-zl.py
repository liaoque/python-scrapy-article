...
if cursor.rowcount:
    ...
    #找到最新的时间的数据
    sql = "select name, code, p_min, p_max, p_start, p_end, p_range, buy_count, buy_sum, date_as from mc_shares where date_as >= '%s' order by date_as asc" % (date_as);
else:
...
if cursor.rowcount:
    ...
    for res in result2:
        p_p_range = 0
        #查找最后的一天的p_end
        sql = "SELECT p_end FROM mc_shares WHERE code = '%s' and date_as < '%s' order by date_as desc limit 1" % (res[1], res[9])
