...
class Items(scrapy.Item):
    ...
    def save(self, cursor):
        ...
        if self.exitsByCode(cursor, code_id, block_code_id):
            return
        sql = """
            INSERT INTO mc_shares_join_block (code_id, block_code_id)
            VALUES (%s, %s)
            """;
        params = (
            code_id,
    ...
    def exitsByCode(self, cursor, code_id, block_code_id):
        sql = "SELECT code_id FROM mc_shares_join_block  WHERE code_id = '%s' and block_code_id='%s'" % (code_id, block_code_id)
        cursor.execute(sql);
