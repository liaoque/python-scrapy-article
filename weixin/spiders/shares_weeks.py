...
import weixin.shares.items_weeks as SharesDateItems
import time
...
class Shares(scrapy.Spider):
    ...
    def findStokByCode(self, code):
        sql = "SELECT code_id as code,name,date_as FROM `mc_shares_weeks` WHERE code_id = '%s' ORDER BY date_as DESC LIMIT 1" % (code);
        results = []
        try:
            # 执行SQL语句
            self.cursor.execute(sql)
